...
import { locations, items } from "./Inwards/consts";
                  ...
                  {Object.keys(items).map((element, i) => {
                    return (
                      <tr key={i} style={{ color: "black" }}>
                        ...
                        <td style={{ verticalAlign: "top" }}>{element}</td>
                        {Object.keys(items[element]).map((value, i1) => {
                          return (
                            <td key={value}>

                              {
                                (items[element][value].totalInwardNet > items[element][value].totalOutwardNet) ?
                                  <span className="badge light badge-success" style={{ fontSize: "16px", color: "green" }}>{
                                    items[element][value].totalInwardNet - items[element][value].totalOutwardNet}</span>
                                  : ""
                              }
                              <br></br>
                              <small style={{ fontSize: "6px" }}>
                                Inwards: <span style={{ fontSize: "12px" }}>{items[element][value].totalInwardNet}</span><br></br>
                                Outwards: <span style={{ fontSize: "12px" }}>{items[element][value].totalOutwardNet}</span>
                              </small>
                            </td>
                          )
                        })}
          ...
          if (response.data.type == "success") {
            ...
            Object.keys(itemsArray).map((element, i) => {
              Object.keys(itemsArray[element]).map((value, i1) => {
                if (!itemHeads.includes(value)) {
                  itemHeads.push(value);
                }
              })
            })
